...
public abstract class Renderer3 {
    ...
    protected Renderer3(RenderTask renderTask) {
            ...
            for (int x = 0; x < 32; x++) {
                for (int z = 0; z < 32; z++) {
                    ...
                    try {
                        ...
                        int northwest = blockY - (northwestChunk?.RainHeightMap[ChunkIndex(offsetX, offsetZ)] ?? blockY);
                        int west = blockY - ((offsetX < 0 ? mapChunkArray[1] : mapChunk)?.RainHeightMap[ChunkIndex(offsetX, z)] ?? blockY);
                        int north = blockY - ((offsetZ < 0 ? mapChunkArray[2] : mapChunk)?.RainHeightMap[ChunkIndex(x, offsetZ)] ?? blockY);
    ...
    private int GetTopBlockY(ServerMapChunk mapChunk, int x, int z) {
        return GameMath.Clamp(mapChunk.RainHeightMap[ChunkIndex(x, z)], 0, Api.WorldManager.MapSizeY - 1);
    }
