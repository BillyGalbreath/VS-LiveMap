...
public class MedievalRenderer : Renderer {
    ...
    public static OrderedDictionary<string, string> HexColorsByCode { get; } = new() {
        { "ink", "#483018" },
        { "settlement", "#856844" },
        { "wateredge", "#483018" },
        { "land", "#AC8858" },
    ...
    public static OrderedDictionary<string, uint> ColorsByCode { get; } = new();
    ...
    private readonly bool[] _blockIsWater;
    ...
    public MedievalRenderer(LiveMapServer server) : base(server, "medieval") {
        ...
        foreach (Block block in server.Api.World.Blocks) {
            ...
            _blockIsWater[id] = IsWater(block);
        }
    }
    ...
    private uint GetMedievalStyleColor(int blockId, int x, int z, BlockData blockData) {
        ...
        if (_blockIsWater[blockId]) {
            ...
            pixelColor = (blockIdN == null || _blockIsWater[blockIdN.Top]) &&
                         (blockIdE == null || _blockIsWater[blockIdE.Top]) &&
                         (blockIdS == null || _blockIsWater[blockIdS.Top]) &&
                         (blockIdW == null || _blockIsWater[blockIdW.Top])
                ? GetColor(blockId)
